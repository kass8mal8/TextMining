...
const Upload = () => {
	...
	const url = "http://localhost:5000/api/doc/upload";

	const handleImageChange = async (e) => {
		const file = e.target.files[0];
		const fileProperties = file.name.split(".");
		setFileDetails(file);
		setFileType(fileProperties[1]);
		setFileName(fileProperties[0]);
		console.log(file);
	...
	const fileSize = Math.floor(fileDetails?.size / 1000);

	return (
		<div className="absolute left-[24%] h-screen top-0 w-[76%]">
